-- Run as the project owner before migration 036. Keep this snapshot inside Supabase.
...
do $$
declare source text;bundle jsonb:='{}';rows jsonb;backup_id text:='before-leadership-036-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSUS');
begin
 foreach source in array array['public.staff','public.manager_profiles','public.training_positions','public.training_signoffs','public.training_sessions','public.change_history','private.qualification_history','private.employee_accounts','private.schedule_weeks','private.schedule_revisions','private.scheduler_notifications','private.scheduler_audit','private.staff_imports'] loop
...
select id,captured_at from private.scheduler_backups where id like 'before-leadership-036-%' order by captured_at desc limit 1;
