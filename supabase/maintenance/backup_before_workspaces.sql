-- Fresh owner-only, repeatable-read snapshot for the workspace upgrade. No passwords, Auth tokens, or service keys.
...
begin
 if exists(select 1 from private.scheduler_backups where id='before-workspaces-20260921') then return;end if;
 ...
 values('before-workspaces-20260921',bundle,
...
commit;
select id,captured_at,(select jsonb_object_agg(key,jsonb_array_length(value)) from jsonb_each(records)) as record_counts from private.scheduler_backups where id='before-workspaces-20260921';
