...
test('successful form save clears only that form; failures and later edits remain protected',()=>{
 const listeners=new Map();let effect;let prompts=0;const forms=[];const signals=[];
 ...
 const context={exports:{},require:()=>({useRef:v=>({current:v}),useEffect:f=>{effect=f}}),HTMLFormElement:Form,Event,CustomEvent:class extends Event{constructor(type,options){super(type);this.detail=options.detail}},MutationObserver:class{observe(){}disconnect(){}},document:{querySelectorAll:()=>forms,body:{},addEventListener:(n,f)=>listeners.set(n,f),removeEventListener:n=>listeners.delete(n)},window:{dispatchEvent:e=>signals.push(e.detail),addEventListener(){},removeEventListener(){}},confirm:()=>{prompts++;return false},setTimeout};
 ...
 assert.equal(canLeave(),false,'not saved or failed save must stay dirty');listeners.get('input')();assert.equal(signals.at(-1),true);
 context.exports.markFormSaved(meeting);
 assert.equal(canLeave(),true,'successful save permits immediate navigation before unmount');assert.equal(signals.at(-1),false);
