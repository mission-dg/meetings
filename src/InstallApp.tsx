...
export function InstallApp(){
 ...
 return <div className="install-app">{offline&&<p className="notice" role="status">You’re offline. Reconnect and refresh before using schedules or saving changes.</p>}{!installed&&<details><summary>Add STARS-S to your phone</summary><p>Open your schedule from your home screen. Internet access is required.</p>{prompt&&<button type="button" onClick={()=>void install()}>Install STARS Scheduling</button>}<p><strong>iPhone / iPad:</strong> Open this site in Safari, tap Share, then Add to Home Screen. If offered, keep Open as Web App enabled.</p><p><strong>Android:</strong> Open this site in Chrome, then choose Install app or Add to Home screen from the browser menu.</p>{error&&<p role="status">{error}</p>}</details>}</div>;
}
