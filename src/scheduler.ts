...
export type SchedulerData={appearance?:import('./scheduleColors').Appearance;schedule_colors?:import('./scheduleColors').ScheduleColors;schedule_colors_error?:string;schedule_meeting_shifts?:Record<string,string>;workspace?:Workspace;available_views?:Workspace[];self:{id:string;person_id:string;staff_id?:string;name:string;is_manager:boolean;is_ca:boolean;is_admin:boolean;is_gm?:boolean};week:{start:string;published_id:string|null;draft:Revision|null};people:Person[];published:{week:string;revision_id:string;version?:number;shifts:WorkShift[]}[];jobs:TrainingPosition[];training:LinkedTraining[];signoffs:TrainingSignoff[];shift_meetings?:ShiftMeeting[];meeting_due?:{staff_id:string;shift_id:string}[];meeting_managers?:{id:string;name:string;person_id:string;is_gm:boolean}[];meeting_requests?:{id:string;staff_id:string;meeting_id:string|null}[];appointments:{timing_mode?:string;id:string;scheduled_at:string;status:string;type:string;manager:string;employee:string}[];requests:ScheduleRequest[];announcements:Announcement[];announcement_history?:{id:number;announcement_id:string;at:string;value:Announcement}[];notifications:{id:string;event_key?:string;body:string;created_at:string;read_at:string|null}[];accounts:{id:string;staff_id:string;active:boolean;is_ca:boolean;version:number;role?:string;manager_version?:number}[];service:{email_verified?:boolean;email_evidence?:string;last_worker_at?:string};releases:{id:string;week:string;state:string;release_at:string;release_name:string;error:string}[];audit:{id:number;actor_name:string;action:string;record_id:string;changed_at:string;before_value:unknown;after_value:unknown}[]};
...
export const shiftLabel=(s:WorkShift,jobs:{id:string;name:string}[])=>s.assignment_type==='training'?s.activity_title||'Training':s.assignment_type==='opening_office'?'Opening office':s.assignment_type==='closing_office'?'Closing office':jobs.find(j=>j.id===s.job_id)?.name||'SHL';
export const hourlyShifts=(shifts:WorkShift[],people:Person[])=>shifts.filter(s=>people.find(p=>p.id===s.person_id)?.employment_type!=='Salaried');

export type ShiftMeeting={id:string;staff_id:string;manager_id:string;manager:string;employee:string;type:'Routine'|'Special';status:string;version:number;created_by:string;work_shift_id:string|null;manager_shift_id:string|null;schedule_revision_id:string|null;timing_mode:'exact'|'during_shift';scheduled_at:string;completed_on:string|null};
