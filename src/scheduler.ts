...
export type WorkShift={assignment_type?:'regular'|'opening_office'|'closing_office';id:string;person_id:string;start:string;end:string;slot:number;job_id:string|null;qualification_reason?:string};
...
export function scheduleChanges(before:WorkShift[],after:WorkShift[]){return {added:after.filter(s=>!before.some(p=>p.id===s.id)),changed:after.filter(s=>before.some(p=>p.id===s.id&&['person_id','start','end','slot','job_id','assignment_type','qualification_reason'].some(k=>p[k as keyof WorkShift]!==s[k as keyof WorkShift]))),removed:before.filter(s=>!after.some(p=>p.id===s.id))}}
...
export const shiftLabel=(s:WorkShift,jobs:{id:string;name:string}[])=>s.assignment_type==='opening_office'?'Opening office':s.assignment_type==='closing_office'?'Closing office':jobs.find(j=>j.id===s.job_id)?.name||'SHL';
