...
function ApprovedHours({data,person}:{data:SchedulerData;person:string}){
 ...
 return <><h3>Current approved hours</h3>{current?<><p className="muted">Effective from {current.payload.effective} · Repeats weekly in Central time</p><AvailabilitySummary days={current.payload.days}/><DecisionDetails request={current}/></>:<p className="muted">No approved availability on file. Scheduling has no recurring availability restrictions yet.</p>}
...
export function AvailabilityOverview({data,busy,onRequest}:{data:SchedulerData;busy:boolean;onRequest:()=>void}){
 ...
 return <div className="availability-overview"><section className="panel scheduler-card"><div className="section-title"><div><h2>My availability</h2><p className="muted">Set the days and times you can work. Changes take effect only after manager approval.</p></div><button className="primary" disabled={busy||!onRoster} onClick={onRequest}>Set availability</button></div>
...
export function AvailabilityFields({data}:{data:SchedulerData}){
 ...
 return <><label>Effective from<input name="effective" type="date" required min={today()} defaultValue={today()}/></label><p className="muted">Weekly availability · Central time. Your current approved hours are prefilled. A pending or denied request does not change them. Use time off for a one-time absence.</p><p className="muted">For overnight hours, set both days. An end time of 12:00 AM means midnight at the end of that day.</p>
