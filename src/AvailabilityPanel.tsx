...
function ApprovedHours({data,person}:{data:SchedulerData;person:string}){
 ...
 return <><h3>Current approved hours</h3>{current?<><p className="muted">Effective from {current.payload.effective}{current.payload.until?' through '+current.payload.until:''} · Repeats weekly in Central time</p><AvailabilitySummary days={current.payload.days}/><DecisionDetails request={current}/></>:<p className="muted">No approved availability on file. Scheduling has no recurring availability restrictions yet.</p>}
...
export function AvailabilityOverview({data,busy,onRequest}:{data:SchedulerData;busy:boolean;onRequest:()=>void}){
 ...
 return <div className="availability-overview">{!data.self.is_manager&&<section className="panel scheduler-card"><div className="section-title"><div><h2>My availability</h2><p className="muted">Set the days and times you can work. Changes take effect only after manager approval.</p></div><button className="primary" disabled={busy||!onRoster} onClick={onRequest}>Set availability</button></div>
