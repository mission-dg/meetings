export type ImportRow={first_name:string;last_name:string;department:'FOH'|'BOH';active:boolean};
export function parseEmployees(text:string):ImportRow[]{
 ...
 if(!headers||headers.length!==4||new Set(headers).size!==4||!['firstname','lastname','department','active'].every(h=>headers.includes(h)))throw Error('Use the four template columns: First Name, Last Name, Department, Active.');
 if(!rows.length||rows.length>500)throw Error('Include between 1 and 500 employees.');
 return rows.map((r,i)=>{const v=(h:string)=>r[headers.indexOf(h)]||'';const f=v('firstname'),l=v('lastname'),d=v('department').toUpperCase(),a=v('active').toLowerCase();
  if(r.length!==4||!f||!l||f.length>100||l.length>100||!['FOH','BOH'].includes(d)||!['true','false','yes','no','1','0',''].includes(a))throw Error(`Row ${i+2}: enter both names, FOH or BOH, and Yes/No for Active (blank means Yes).`);
  return {first_name:f,last_name:l,department:d as 'FOH'|'BOH',active:!['false','no','0'].includes(a)};
 });
}
