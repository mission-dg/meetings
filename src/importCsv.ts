export type ImportRow={first_name:string;last_name:string;department:'FOH'|'BOH'|'Catering';active:boolean;staff_id?:string;primary_role?:string;other_roles?:string[];hourly_wage?:string;choice?:string;target_id?:string};
export function parseEmployees(text:string):ImportRow[]{
 ...
 return rows.map((r,i)=>{const v=(h:string)=>r[headers.indexOf(h)]||'';const f=v('firstname'),l=v('lastname'),d=v('department').toUpperCase()==='CATERING'?'Catering':v('department').toUpperCase()==='HOH'?'BOH':v('department').toUpperCase(),a=v('active').toLowerCase();
  if(r.length!==headers.length||(!v('staffid')&&(!f||!l))||f.length>100||l.length>100||!['','FOH','BOH','Catering'].includes(d)||!['true','false','yes','no','1','0',''].includes(a))throw Error(`Row ${i+2}: enter both names, FOH, HOH, or Catering (SHLs are added through SHL accounts), and Yes/No for Active (blank means Yes).`);
  return {first_name:f,last_name:l,department:d as 'FOH'|'BOH'|'Catering',active:!['false','no','0'].includes(a),...(headers.includes('staffid')?{staff_id:v('staffid')}:{}),...(headers.includes('primaryrole')?{primary_role:v('primaryrole')}:{}),...(headers.includes('otherroles')?{other_roles:[...new Set(v('otherroles').split(',').map(x=>x.trim()).filter(Boolean))]}:{}),...(headers.includes('hourlywage')?{hourly_wage:v('hourlywage')}:{})};
 });
}
