...
export function WorkspaceRoot({legacy}:{legacy:(back:(view?:string,staff?:string,job?:string)=>void,authors:Record<string,string>)=>ReactNode}){
 ...
 if(old&&workspace!=='employee')return legacy((next,staff,job)=>{setView(next||'home');setOld(false);if(next==='training'&&staff)setTraining({staff,job});void load().catch(()=>{})},Object.fromEntries(data.accounts.map(a=>[a.id,data.people.find(p=>p.staff_id===a.staff_id)?.name||'Former teammate'])));
 ...
 {view==='schedule'&&<><ScheduleBoard reload={load} preview={preview} data={data} week={week} setWeek={setWeek} act={act} busy={busy} onTraining={revision=>setTraining({revision})}/><Commitments data={data}/></>}
