...
export function WorkspaceRoot({legacy}:{legacy:(back:(view?:string,staff?:string,job?:string,week?:string)=>void,authors:Record<string,string>)=>ReactNode}){
 ...
 if(old&&workspace!=='employee')return legacy((next,staff,job,meetingWeek)=>{if(meetingWeek)setWeek(meetingWeek);setView(next||'home');setOld(false);if(next==='training'&&staff)setTraining({staff,job});void load().catch(()=>{})},Object.fromEntries(data.accounts.map(a=>[a.id,data.people.find(p=>p.staff_id===a.staff_id)?.name||'Former teammate'])));
 ...
 {workspace!=='employee'&&<aside><div className="brand"><span className="brand-icon">★</span><span>STARS Scheduling<small>{workspace==='it'?'SYSTEM ADMINISTRATION':'MANAGER WORKSPACE'}</small></span></div>{switcher}<nav aria-label={workspaceLabels[workspace]+' navigation'}>{navigation}</nav><div className="sidebar-bottom"><AccountMenu name={access.name} onPassword={()=>{if(canLeave())setPassword(true)}}/><button onClick={()=>{if(canLeave())void signOut()}}>Sign out</button></div></aside>}
 <main><header className="topbar"><div className="workspace-heading">{workspace==='employee'?<strong>STARS Scheduling <span>My Shift</span></strong>:<strong>{workspaceLabels[workspace]}</strong>}{workspace==='employee'?<AccountMenu name={access.name} onPassword={()=>{if(canLeave())setPassword(true)}}/>:<small>{access.name}</small>}</div><div className="actions">{workspace==='employee'&&switcher}<button aria-label="Refresh workspace" disabled={busy} onClick={()=>void load().catch(()=>{})}><RefreshCw size={18}/></button>{workspace==='employee'&&<button onClick={()=>{if(canLeave())void signOut()}}>Sign out</button>}</div></header>
 ...
 {view==='staffMeetings'&&<StaffMeetings employee userId={data.self.id} preview={preview}/>}{view==='schedule'&&<><ScheduleBoard reload={load} preview={preview} data={data} week={week} setWeek={setWeek} act={act} busy={busy} onTraining={revision=>setTraining({revision})}/><Commitments data={data}/></>}
 ...
 {view==='announcements'&&<Announcements data={data} act={act} busy={busy} onRequest={id=>{if(busy||!canLeave())return;setRequestTarget(id);setView('requests');setError('');setNotice('');const url=new URL(location.href);url.searchParams.set('page','requests');history.replaceState(null,'',url)}}/>}
