...
export function WorkspaceRoot({legacy}:{legacy:(back:(view?:string,staff?:string,job?:string)=>void,authors:Record<string,string>)=>ReactNode}){
 ...
 <main><header className="topbar"><div className="workspace-heading">{workspace==='employee'?<strong>STARS Scheduling <span>My Shift</span></strong>:<strong>{workspaceLabels[workspace]}</strong>}<small>{access.name}</small></div><div className="actions">{workspace==='employee'&&switcher}<button aria-label="Refresh workspace" disabled={busy} onClick={()=>void load().catch(()=>{})}><RefreshCw size={18}/></button>{workspace==='employee'&&<button onClick={()=>{if(canLeave())void signOut()}}>Sign out</button>}</div></header>
