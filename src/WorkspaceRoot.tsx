...
export function WorkspaceRoot({legacy}:{legacy:(back:(view?:string,staff?:string,job?:string,week?:string)=>void,authors:Record<string,string>)=>ReactNode}){
 ...
 if(!session&&!preview)return <div className="login"><div className="login-story"><div className="brand"><span className="brand-icon">★</span><span>STARS Scheduling<small>TEAM WORKSPACE</small></span></div><div><p className="eyebrow">TOGETHER, EVERY SHIFT</p><h1>Your team.<br/>Your next shift.</h1><p>Scheduling Our Spectacular Teammates Achieving Remarkable Significance</p></div><small>Downers Grove, Illinois · Private team access</small></div><section className="login-form"><p className="eyebrow">TEAM WORKSPACE</p><h2>Welcome back.</h2><p className="mobile-stars-tagline">Scheduling Our Spectacular Teammates Achieving Remarkable Significance</p><p className="muted">Sign in with your username or email and password.</p>{configured?<LoginForm/>:<p>The workspace is being connected.</p>}<InstallApp/></section></div>;
